...
//! Library for the crime data source discovery tool.
//!
//! Provides command handlers, helper utilities, and the interactive TUI for
//! tracking leads, managing confirmed sources, recording search history,
//! reviewing legal/licensing info, managing scrape targets, seeding the
//! database with existing knowledge, health-checking sources, and suggesting
...
pub struct SeedLead {
    /// Jurisdiction name (e.g., "Norfolk, VA").
    pub jurisdiction: &'static str,
    /// Human-readable name for the data source.
    pub name: &'static str,
    /// API type (e.g., "socrata", "arcgis").
    pub api_type: &'static str,
    /// URL for the data source endpoint.
    pub url: &'static str,
...
// ---------------------------------------------------------------------------
// CLI definitions (shared between main.rs and interactive.rs)
// ---------------------------------------------------------------------------
...
pub async fn cmd_status(database: &dyn Database) -> Result<(), Box<dyn std::error::Error>> {
...
///
/// Returns an error if the database query fails.
pub async fn cmd_leads_investigate(
    database: &dyn Database,
    id: i64,
) -> Result<(), Box<dyn std::error::Error>> {
...
///
/// Returns an error if the database query fails.
pub async fn cmd_legal_list(database: &dyn Database) -> Result<(), Box<dyn std::error::Error>> {
...
/// # Errors
///
/// Returns an error if any of the seed operations fail.
pub async fn cmd_seed(database: &dyn Database) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn seed_sources(database: &dyn Database) -> Result<u32, Box<dyn std::error::Error>> {
...
#[allow(clippy::too_many_lines)]
pub async fn seed_leads(database: &dyn Database) -> Result<u32, Box<dyn std::error::Error>> {
...
/// Returns an error if the database operations fail.
#[allow(clippy::too_many_lines)]
pub async fn seed_searches(database: &dyn Database) -> Result<u32, Box<dyn std::error::Error>> {
...
///
/// Returns an error if the database operations fail.
pub async fn seed_patterns(database: &dyn Database) -> Result<u32, Box<dyn std::error::Error>> {
...
pub fn extract_source_info(
    fetcher: &crime_map_source::source_def::FetcherConfig,
) -> (&str, String) {
...
/// # Errors
///
/// Returns an error if the lead cannot be found or the file operations fail.
pub async fn cmd_integrate(
    database: &dyn Database,
    id: i64,
...
#[must_use]
pub fn derive_source_id(jurisdiction: &str) -> String {
...
#[must_use]
pub fn extract_carto_table(url: &str) -> String {
...
#[must_use]
pub fn extract_carto_base(url: &str) -> String {
    url.find('?')
...
pub fn insert_registry_entry(
    ...
    for (i, line) in lines.iter().enumerate().skip(section_idx + 1) {
        ...
        let is_section_header =
            trimmed.starts_with("//") && (trimmed.contains('\u{2500}') || trimmed.contains("--"));
...
#[must_use]
pub fn increment_source_count(content: &str) -> String {
...
/// # Errors
///
/// Returns an error if the health-check operations fail.
...
/// Truncates a string to `max_len` characters, appending "\u{2026}" if it was
...
#[must_use]
pub const fn bool_str(value: Option<bool>) -> &'static str {
    match value {
        Some(true) => "Yes",
        Some(false) => "No",
        None => "-",
